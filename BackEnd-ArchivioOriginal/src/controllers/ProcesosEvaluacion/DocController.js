...
export const obtenerDocs = async (req, res) => {
    try {
        let sql = "SELECT * FROM procesos_evaluacion";
...
export const obtenerMejora = async(req, res) => {
    try{
        const {categoria, categoria2} = req.params
        let sql = `select * from procesos_evaluacion where categoria = ? and categoria2 = ?`
...
export const obtenerEvaluacionIndependiente = async(req, res) => {
    try{
        const {categoria} = req.params
        let sql = `select * from procesos_evaluacion where categoria = ? `
...
export const actualizarPrEvaluacion = async(req, res) => {
    try {
        let { id_evaluacion } = req.params;
        ...
        let sql = `UPDATE procesos_evaluacion SET fk_id_usuario = ?, categoria = ?, categoria2 = ?, nombre_documento = ?, descripcion = ?, formato = ?, estado = ?, fecha_carga = ?, documento = ? WHERE id_evaluacion = ?`;

        const values = [fk_id_usuario, categoria, categoria2, nombre_documento, descripcion, formato, estado, fecha_carga, documento, id_evaluacion];
...
export const eliminarPrEvaluacion = async (req, res) => {
    try {
        let { id_evaluacion } = req.params;

        let sql = `DELETE FROM procesos_evaluacion WHERE id_evaluacion = ?`;

        const [resultado] = await conexion.query(sql, [id_evaluacion]);
...
export const subrDocs = async (req, res) => {
    try {
        ...
        let sql = `INSERT INTO procesos_evaluacion (fk_id_usuario, categoria, categoria2, nombre_documento, descripcion, formato, estado, fecha_carga, documento)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        ...
        if (respuesta.affectedRows > 0) {
            const id_evaluacion = respuesta.insertId
...
export const estadoDocumento = async (req, res)=> {
    ...
    try{
       const sql = 'select estado, fk_id_usuario, nombre_documento from procesos_evaluacion where id_evaluacion = ?'
       const [estadoresult] = await conexion.query(sql,[id_evaluacion])
       ...
       const actualizarSql = `update procesos_evaluacion set estado = ? where id_evaluacion = ?`

       await conexion.query(actualizarSql,[NewEstado, id_evaluacion])
